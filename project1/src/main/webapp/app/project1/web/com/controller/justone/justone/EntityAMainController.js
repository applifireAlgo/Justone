Ext.define('Project1.project1.web.com.controller.justone.justone.EntityAMainController', {
     extend: 'Project1.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.EntityAMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#EntityATree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#EntityAForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#EntityAGrid');
               var tree = this.view.down('#EntityATree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#EntityAForm');
          var grid = this.view.down('#EntityAGrid');
          var tree = this.view.down('#EntityATreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#EntityAGrid');
               ...
               var formPanel = this.getView().up().down('#EntityAForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#EntityATree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#EntityAForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#EntityAGrid').store;
          gridStore.removeAll();
